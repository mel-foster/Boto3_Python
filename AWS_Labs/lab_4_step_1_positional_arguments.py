...
def translate_text(text, source_language_code, target_language_code): # We deine the positional arguments in the ()
    client = boto3.client('translate')
    response = client.translate_text(
        Text=text, #we remove the hard coded value 
        SourceLanguageCode=source_language_code,  # We used positional arguments instead
